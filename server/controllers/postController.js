...
const createPost = async (req, res) => {
  ...
  // console.log("User Account Type:", req.accountType);

  if (authorAccountType == "buyer") {
    return res
      .status(403)
      .json({ success: false, message: "Forbidden, only seller can post" });
  }
  ...
  try {
    ...
    return res.status(201).json({ success: true, message: "Post created succesfully", post});
  } catch (error) {
...
const deletePost = async (req, res) => {
  ...
  try {
    ...
    // we will not do this as some of the people had already purchased our asset
...
const getFavourites = async (req, res) => {
    ...
    try {
        ...
        if( !favourites)
            return res
               .status(404)
               .json({ success: false, message: "No favourite added"});
